...
public class UserServiceProvider {// extends ServletContainer {
    ...
    public Response getUserById(@PathParam("id") int id) {
        System.err.println("test get user" + id);

        Optional<User> userOptional = userService.findUserByID(id);
        System.err.println("1!!!!!!!!!!" + userService.hashCode());
    ...
    public Response patch(List<PatchParameter> params) {
        ...
        return Response.ok(new GenericEntity<List<UserInfo>>(UserInfo.fromUsers(users)) {
        }).build();
    }
