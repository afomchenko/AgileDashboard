 ...
 */
@Component(name = "ru.mera.agileboard.service.LoggingComponent", immediate = true)
@Service(value = ru.mera.agileboard.service.LoggingService.class)
public class LoggingServiceImpl implements LoggingService {
    ...
    public List<TaskLog> getRecentLogByUser(User user) {
        ...
        try {
            ...
            qLog.where().eq("user_id", user.getId()).and().gt("log_date", LocalDateTime.now().minus(7, ChronoUnit.DAYS).truncatedTo(ChronoUnit.DAYS).atZone(ZoneId.systemDefault()).toEpochSecond() * 1000);
    ...
    public Map<Task, Integer> getLoggedByTask(Collection<? extends Task> tasks) {
        ...
        try {
            ...
            qLog.selectRaw("ab_tasks.task_id, IFNULL(SUM(ab_task_logs.logged),0) ").leftJoin(dao.queryBuilder()).groupBy("task_id").where().in("task_id", tasks.stream().map(Task::getId).collect(Collectors.toList()));
            System.err.println(qLog.prepare().toString());
